...
class IncidentFormComponent extends HTMLElement {
  ...
  _addEventListeners() {
    ...
    const statusOptions = this.shadowRoot.querySelectorAll(
      'input[name="status"]'
    );
    ...
    if (reporterInput)
      reporterInput.addEventListener("input", this._handleInput);
    if (descriptionInput)
      descriptionInput.addEventListener("input", this._handleInput);
  ...
  _removeEventListeners() {
    ...
    if (reporterInput)
      reporterInput.removeEventListener("input", this._handleInput);
    if (descriptionInput)
      descriptionInput.removeEventListener("input", this._handleInput);
  ...
  async _loadIncidentData() {
    try {
      ...
      if (incident) {
        ...
        // Mostrar error
        store.dispatch(
          actions.showErrorNotification("No se encontró el incidente")
        );
        this._handleCancel();
      }
    } catch (error) {
    ...
    switch (name) {
      ...
      case "description":
        if (!value.trim()) {
          ...
          value.trim().length <
          CONFIG.VALIDATION.INCIDENT_DESCRIPTION_MIN_LENGTH
        ) {
  ...
  _updateForm() {
    ...
    if (formTitle) {
      formTitle.textContent =
        this._mode === "create" ? "Crear Nuevo Incidente" : "Editar Incidente";
    }
  ...
  async _handleSubmit(event) {
    ...
    try {
      ...
      if (this._mode === "create") {
        ...
        store.dispatch(
          actions.showSuccessNotification("Incidente creado con éxito")
        );
      } else {
        ...
        // Mostrar notificación
        store.dispatch(
          actions.showSuccessNotification("Incidente actualizado con éxito")
        );
      }

      // Emitir evento de éxito
      this.dispatchEvent(
        new CustomEvent("incident-form:submit", {
          bubbles: true,
          composed: true,
          detail: {
            incident: response,
      ...
      store.dispatch(
        actions.showErrorNotification(
          `Error al ${
            this._mode === "create" ? "crear" : "actualizar"
          } incidente: ${error.message || "Inténtalo de nuevo"}`
        )
      );
    } finally {
      this._isSubmitting = false;
  ...
  _handleCancel() {
    ...
    this.dispatchEvent(
      new CustomEvent("incident-form:cancel", {
        bubbles: true,
        composed: true,
      })
    );
        ...
        <form hidden="${this._isLoading}">
          <div class="form-group">
            ...
            <input 
              ...
              required
            >
            <div class="error-message reporter-error" hidden="${!this
              ._formErrors.reporter}">
          ...
          <div class="form-group">
            ...
            <textarea 
              id="description" 
              class="input-field ${
                this._formErrors.description ? "error" : ""
              }" 
              name="description"
              ?disabled="${this._mode === "edit"}"
              required
            ...
            <div class="error-message description-error" hidden="${!this
              ._formErrors.description}">
            ...
            <div class="status-options">
              <label class="status-option">
                <input type="radio" name="status" value="${
                  ...
                  ?checked="${
                    this._formData.status === CONFIG.INCIDENT_STATUS.PENDING
                  }">
                Pendiente
              </label>
              <label class="status-option">
                <input type="radio" name="status" value="${
                  ...
                  ?checked="${
                    this._formData.status === CONFIG.INCIDENT_STATUS.IN_PROGRESS
                  }">
                En proceso
              </label>
              <label class="status-option">
                <input type="radio" name="status" value="${
                  ...
                  ?checked="${
                    this._formData.status === CONFIG.INCIDENT_STATUS.RESOLVED
                  }">
                Resuelto
              </label>
            </div>
