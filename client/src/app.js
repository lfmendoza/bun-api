...
window.addEventListener("unhandledrejection", (event) => {
  console.warn("Promesa no manejada:", event.reason);
  event.preventDefault(); // Evitar que el error se muestre en la consola
});

// Manejador global para errores no capturados
window.addEventListener("error", (event) => {
  console.warn("Error no capturado:", event.error);

  // Sólo en desarrollo, en producción sería mejor usar un servicio de monitoreo
  if (window.location.hostname === "localhost") {
    // No prevenir el comportamiento por defecto en producción
